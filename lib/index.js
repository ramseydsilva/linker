...
var Linker = function(options) {
    this.defaults = {
        ...
        unallowedExtensions: ['.pdf', '.jpeg', '.jpg', '.bmp', '.gif'],
        allowedDomains: [],
        sameDomain: true,
        ...
        src: [jquery],
            ...
            if (err) {
                ...
                if (pageLinks.length > that.defaults.maxLinks) {
                    ...
                    var links = _.filter(pageLinks, function(pageLink) { return that.getAbsoluteUrl(that.website.href, $(pageLink).attr('href')) });
                    links = _.map(links, function(pageLink) {
...
Linker.prototype.processLink = function(link) {
    ...
    if (link.depth > this.defaults.maxDepth) {
        ...
        promise.resolve();
    } else if (new RegExp(this.defaults.unallowedExtensions.join("|")).test(link.href)) {
        link.reason = 'UnAllowed extension';
        promise.resolve();
    } else if (this.linkCrawledIndex.indexOf(link.href) != -1) {
...
Linker.prototype._crawl = function(website) {
    ...
    request(website.href).bind(this)
...
if (process.argv.length > 2) {
    ...
    .finally(function(res) {
        console.log('Done', JSON.stringify(l.results, undefined, 2));
    });
