...
Linker.prototype.getLinks = function(link) {
    if (this.defaults.getLinks) {
        ...
        if (!!match && !matchBlacklisted) {
            var result = {source: match[0], href: link};
            ...
            if (err) {
                ...
                var links = _.each(pageLinks, function(pageLink) {
                   if (!!$(pageLink).attr('href')) {
                        ...
                        if (!!href) {
                            that.getLinks(href);
                            if (that.crawledLinks.indexOf(href) < 0) {
                                count++;
                            }
                            if(that.results.length < 5 && count < that.defaults.maxLinks) {
                                var link = {
                                    ...
                                    links: [],
                                    crawled: false,
                                    shouldCrawl: that.crawledLinks.indexOf(href) < 0
                                };
